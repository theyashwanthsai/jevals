...
class Grade:
    ...
    def __str__(self):
        ...
        return f"{self.rubric:<22} {v:<12} conf {self.confidence:.2f}  {flag}{marks}"


class Judge:
    """Grades a case against rubrics in a single Jev call.

    escalate_below: confidence under this triggers the escalator.
    escalator: callable(state, rubric, grade) -> dict | None
    ...
    def grade(self, case, rubrics):
        """-> (list[Grade], meta)"""
        by_name = {r.name: r for r in rubrics}
        if len(by_name) != len(rubrics):
            raise ValueError("duplicate rubric names")
        state = case.state()
        questions = {r.name: r.to_question() for r in rubrics}
        answers, meta = self.client.decide(state, questions)

        grades = []
        for name, rubric in by_name.items():
            if name not in answers:
                raise RuntimeError(f"Jev returned no answer for rubric {name!r}")
            a = answers[name]
            value, conf, passed, borderline = rubric.read(a)
            g = Grade(case=case.id, rubric=name, type=rubric.type, value=value,
                      confidence=conf, passed=passed, borderline=borderline,
                      probabilities=a.get("probabilities"),
                      weight=rubric.weight, critical=rubric.critical)
            ...
            if self.escalator and (conf < self.escalate_below or borderline):
                ...
                verdict = self.escalator(state, rubric, g)
                g.escalated = True
                if verdict:
